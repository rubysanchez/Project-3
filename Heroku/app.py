...
def hospitaldata():
    ...
    for Facility_Name, Facility_ID, City, State, Latitude, Longitude, Pct_Hospital_Acquired_Infections, Total_Numerator, Total_Denominator in results:
        ...
        data_dict["pct_infections"] = float(Pct_Hospital_Acquired_Infections.strip("%"))
...
def hospitalmeasures():
    ...
    for Facility_Name, Facility_ID, State, Pct_Catheter_Associated_Urinary_Tract_Infections, Pct_Central_Line_Associated_Bloodstream_Infections, Pct_Clostridium_Difficile_CDiff_Infections, Pct_MRSA_Bacteremia_Infections, Pct_Abdominal_Hysterectomy_Infections, Pct_SSI_Colon_Surgery_Infections in results:
        ...
        measures_dict["pct_cath_UTI"] = float(Pct_Catheter_Associated_Urinary_Tract_Infections)
        measures_dict["pct_cntrl_line"] = float(Pct_Central_Line_Associated_Bloodstream_Infections)
        measures_dict["pct_cdiff"] = float(Pct_Clostridium_Difficile_CDiff_Infections)
        measures_dict["pct_mrsa"] = float(Pct_MRSA_Bacteremia_Infections)
        measures_dict["pct_ab_hyst"] = float(Pct_Abdominal_Hysterectomy_Infections)
        measures_dict["pct_ssi_colon"] = float(Pct_SSI_Colon_Surgery_Infections)
...
def hospitaltopten():
    ...
    for Facility_Name, Facility_ID,  Pct_Hospital_Acquired_Infections, Total_Numerator, Total_Denominator in results:
        ...
        topten_dict["pct_infections"] = float(Pct_Hospital_Acquired_Infections.strip("%"))
...
def hospitalbottomten():
    ...
    for Facility_Name, Facility_ID,  Pct_Hospital_Acquired_Infections, Total_Numerator, Total_Denominator in results:
        ...
        bottomten_dict["pct_infections"] = float(Pct_Hospital_Acquired_Infections.strip("%"))
