...
// Looping through the hospitals array, create one marker for each hospital, bind a popup containing its name and percent infection, and add it to the map.
for (var i = 0; i < hospitals.length; i++) {
    var hospital = hospitals[i];
    L.marker([hospital.lat, hospital.lng]).bindPopup(`<h4>${hospital.name}</h4> <hr> <h5>Percent Infection ${hospital.pct_infections}</h5>`)
        .addTo(myMap);
};